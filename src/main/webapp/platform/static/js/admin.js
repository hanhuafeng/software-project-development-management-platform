...
layui.define(['jquery', 'form', 'layer', 'element'], function (exports) {
    ...
    /*
        ...
        h       弹出层高度（缺省调默认值）
    */
    window.WeAdminShow = function (title, url, w, h,full) {
        if (title == null || title == '') {
            title = false;
        }
        ;
    ...
    window.rePassword = function (id) {
        layer.confirm('确定重置密码吗？', function (index) {
            layer.close(index);
            $.ajax({
                type: "GET",
                url: ADMIN_RESET_PASSWORED + '/' + id,
                dataType: "json",
                async: false,
                success: function (data) {
                    layer.msg("重置成功!重置后的密码为123456!",{
                        icon:1,
                        time:5000
                    })
                },
